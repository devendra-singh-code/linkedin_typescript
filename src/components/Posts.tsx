...
const Posts = ({ posts }: { posts: any }) => {
  ...
  const commentPost = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/commentOnPost",
        { postId: posts._id, comments: commentText },
  ...
  const sendRequest = async () => {
    try {
      const response = await axios.post(
        "/api/friends/sendRequests",
        { recevierId: posts.createdBy._id },
        { withCredentials: true }
      );
      if (response.data.success) {
        toast.success(response.data.message);
  ...
  return (
    <>
      ...
      <div className="relative  dark:bg-[#2b2b2b] rounded-xl flex flex-col gap-3 border border-gray-300">
        <div className="absolute flex items-center gap-5 right-5 top-4 cursor-pointer">
          ...
          {!followed && (
            <div className="bg-blue-100 hover:bg-blue-200 text-blue-700 font-semibold px-3 py-1 rounded-2xl flex items-center gap-1 cursor-pointer transition-all duration-200 shadow-sm">
              <p>Unfollow</p>
            </div>
          )}
        ...
        <div className="px-4 pt-4 flex flex-col gap-3">
          <Link href={`/profile/${posts.createdBy._id}`}>
            <div className="flex gap-4 items-center cursor-pointer">
              {posts?.createdBy?.profile_image ? (
                ...
                <div className="bg-white border-2 border-gray-600 rounded-full w-12 h-12 flex items-center justify-center font-semibold">
                  {posts?.createdBy?.full_name?.charAt(0)}
                </div>
              )}

              <div>
                <p className="font-semibold text-gray dark:text-white text-[14px] ">
          ...
          <div>
            <p className="text-[14px] dark:text-white text-gray-700 leading-4">
        ...
        <div className="w-full ">
          ...
          {posts?.post_video ? (
            <video
              ...
              controls
              className="max-h-[400px] w-full text-center"
            />
          ) : null}
        </div>
        <div className="flex items-center justify-between px-4">
          <div></div>
          <div>
            <p className="text-[14px] font-semibold dark:text-white text-gray-600">
        ...
        <div className="flex items-center text-[14px] justify-between px-4 py-4 border-t border-gray-200 ">
          <div
            onClick={() => likedPost(posts._id)}
            className={`flex items-center gap-2   cursor-pointer ${
              like ? "text-blue-600" : "text-gray-800 dark:text-white"
            }`}
          >
            <ThumbsUp
              className={`  rounded-full  ${
                like ? "bg-blue-600  p-1 w-5 h-5 text-white" : "w-4 h-4"
              }`}
            />

            <p className="font-semibold dark:text-white text-sm">Like</p>
          </div>
          <div
          ...
          <div className="flex items-center gap-2 text-gray-800 dark:text-white cursor-pointer">
            ...
            <div className="flex items-center gap-3 w-full px-4 pb-4 ">
              ...
              <div className="flex items-center border-2 border-gray-300 rounded-full px-3 w-full max-w-lg bg-white dark:bg-gray-900">
                <input
                  ...
                  className="flex-1 bg-transparent  text-sm focus:outline-none h-10 dark:placeholder:text-white dark:text-white"
                  onChange={(e: any) => setCommentText(e.target.value)}
                />
                <button type="button" className="p-1 hover:text-blue-600">
                  <SmileIcon className="w-5 h-5 dark:text-white" />
                </button>
                <button
                  onClick={commentPost}
                  type="button"
                ...
                >
                  <SendHorizonal className="w-5 h-5 dark:text-white" />
                </button>
              </div>
            </div>
